...
### Other visualizations (docs/ only - static build, not in the Express app)

Four more pages share `docs/graph-core.js`, `docs/styles.css`, and the
`docs/nav.js` top nav:

- **`docs/timeline.html`** - the classic "arc diagram" of cross-references:
  every verse placed on one axis, cross-references drawn as arcs above a
  baseline. Two axis modes: canonical (Genesis→Revelation) reading order, or
  estimated year (`data/chronology.json`, `GraphCore.buildChronologyIndex`/
  `yearForPosition` in graph-core.js) - Genesis 1-11 follows Septuagint (LXX)
  genealogical reckoning anchored to Abraham's conventional birth year, later
  books use standard regnal/composition-date estimates; every year is an
  approximate illustrative estimate, see the in-page footer. Drag to pan,
  scroll to zoom, click an arc for a detail panel (both verses' full text),
  click a book band or use the book dropdown to filter.
- **`docs/chiasm.html`** - an explorer for hand-curated chiastic (A-B-C-...-C'-B'-A')
  structures (`data/chiasms.json`): the Flood narrative, Leviticus, Jonah, and
  Revelation, rendered as an indented "hourglass" so nesting depth is visible
  at a glance, each section's ref expandable to verse text. Chiasm identification
  is interpretive - see the in-page disclaimer.
- **`docs/ontology.html`** - `data/ontology.json`, a curated "divine council
  worldview" concept ontology (Divine Council, Angel of the LORD, Melchizedek,
  Rephaim/Nephilim, covenant promises, ~50 Messianic prophecy correlations,
  typological patterns of grace) after Michael Heiser and Douglas Van Dorn, in
  two views: an expandable tree, or a collapsible force-directed node graph
  (click a node to expand/collapse its children and see detail - same physics
  pattern as the main graph). The same data exports as real OWL/RDF Turtle
  (`docs/data/bible-theology-ontology.ttl`, generated by `scripts/build-static.js`).
  This is one particular, named theological reading tradition, not doctrinally
  neutral consensus - see the in-page disclaimer.
- **`docs/textual-criticism.html`** - `data/textual-variants.json`, ~17 curated,
  well-documented differences between the Masoretic Text, Septuagint, and Dead
  Sea Scrolls (Deut. 32:8-9's "sons of Israel"/"sons of God", Goliath's height,
  the Jeremiah recensions, Psalm 145's missing acrostic line, etc.), filterable
  by category, each ref expandable to verse text. The main graph's verse detail
  panel (`docs/app.js`) cross-links here automatically (`findVariantForVerse`)
  whenever the open verse matches a curated variant. Not an exhaustive catalog
  of textual criticism - see the in-page disclaimer.
