...
Three more pages share `docs/graph-core.js`, `docs/styles.css`, and the
`docs/nav.js` top nav:

- **`docs/timeline.html`** - the classic "arc diagram" of cross-references:
  every verse placed on one axis by canonical (Genesis→Revelation) reading
  order, cross-references drawn as arcs above a baseline. Drag to pan, scroll
  to zoom, hover an arc for its two endpoints. Not a historical/composition-date
  timeline - see the in-page caption.
...
- **`docs/ontology.html`** - an expandable-tree browser for `data/ontology.json`,
  a curated "divine council worldview" concept ontology (Divine Council, Angel
  of the LORD, Melchizedek, Rephaim/Nephilim, covenant promises, ~50 Messianic
  prophecy correlations, typological patterns of grace) after Michael Heiser and
  Douglas Van Dorn. The same data exports as real OWL/RDF Turtle
